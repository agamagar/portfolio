...
// Google Material 3 motion easings (cubic-bezier control points) — the site's
// global motion tokens, baked in here so the board matches the rest of the site.
const M3_SPATIAL_EXPRESSIVE: [number, number, number, number] = [0.38, 1.21, 0.22, 1];
const M3_SPATIAL_STANDARD: [number, number, number, number] = [0.27, 1.06, 0.18, 1];
const M3_EFFECTS: [number, number, number, number] = [0.34, 0.8, 0.34, 1];
        ...
        >
          ...
          {flipId > 0 && (
            <motion.div
              ...
              initial={{ opacity: 0.5 }}
              animate={{ opacity: 0 }}
              transition={{ duration: flipDuration * 1.3, ease: M3_EFFECTS }}
            />
          )}
        </div>
